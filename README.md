...
- 🎬 **Phân tích Video (Deepfake Detection):** Ứng dụng mô hình mạng nơ-ron chập 3D (3D ResNet) để trích xuất đặc trưng theo cả không gian (spatial) và thời gian (temporal) nhằm phát hiện video bị cắt ghép, làm giả.
- 🔗 **Multimodal Fusion:** Kết hợp sức mạnh của đa luồng dữ liệu (Text + Image) để cho ra một Độ tin cậy (Confidence Score) chính xác nhất đối với tin tức.
...
![User Requirement Diagram](frontend/public/user_requirements.jpg)
...
graph TD
    ...
    Upload --> |HTTP POST /api/predict & /api/predict_video| API
    
    API <--> |Verify Token| Auth
    API --> |Save Image| Bucket
    API --> |Forward Text + Image/Video| AI
    ...
    VideoModel --> |Return FAKE/REAL & Confidence| Logic
